...
test_data = [
    ...
    (rng.RNG.uniform(-1000, 1000, size=(5, 20)).astype(np.float64), 5),
]
...
def test_multi_mass_absolute_seeded():
    with rng.fix_state():
        T = rng.RNG.uniform(-1000, 1000, size=(3, 10)).astype(np.float64)
        m = 5
...
def test_naive_maamp():
    T = rng.RNG.uniform(-1000, 1000, size=(1, 1000)).astype(np.float64)
    m = 20
...
def test_constant_subsequence_self_join():
    ...
    T = np.array([T_A, T_A, rng.RNG.random(T_A.shape[0])])
    m = 3

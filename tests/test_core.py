...
test_data = [
    ...
    (rng.RNG.uniform(-1000, 1000, size=8), rng.RNG.uniform(-1000, 1000, size=64)),
]
...
def test_check_bad_dtype():
    for dtype in [np.int32, np.int64, np.float32]:
        with pytest.raises(TypeError):
            core.check_dtype(rng.RNG.random(10).astype(dtype))


def test_check_dtype_float64():
    assert core.check_dtype(rng.RNG.random(10))
...
def test_rolling_std_2d():
    ...
    for n_rows in range(1, 4):
        a = rng.RNG.random(size=(n_rows, 64))
        ref_std = naive.rolling_nanstd(a, w)
...
def test_compute_mean_std_multidimensional(Q, T):
    ...
    Q = np.array([Q, rng.RNG.uniform(-1000, 1000, size=Q.shape[0])])
    T = np.array([T, T, rng.RNG.uniform(-1000, 1000, size=T.shape[0])])
...
def test_merge_topk_PI_without_overlap():
    ...
    n = 50
    for k in range(1, 6):
        PA = rng.RNG.random(size=(n, k))
        PA[:, :] = np.sort(PA, axis=1)  # sorting each row separately

        PB = rng.RNG.random(size=(n, k))
        col_idx = rng.RNG.integers(0, k, size=n)
...
def test_merge_topk_PI_with_overlap():
    ...
    for k in range(1, 6):
        ...
        num_overlaps = rng.RNG.integers(1, k + 1, size=n)
        for i in range(n):
            # create overlaps
...
def test_merge_topk_PI_with_1D_input():
    ...
    n_overlaps = rng.RNG.integers(1, n + 1)
...
def test_merge_topk_ρI_without_overlap():
    ...
    n = 50
    for k in range(1, 6):
        ρA = rng.RNG.random(size=(n, k))
        ρA[:, :] = np.sort(ρA, axis=1)  # sorting each row separately

        ρB = rng.RNG.random(size=(n, k))
        col_idx = rng.RNG.integers(0, k, size=n)
        for i in range(n):  # creating ties between values of PA and PB
...
def test_check_P():
    with pytest.raises(ValueError):
        core._check_P(rng.RNG.random(size=(2, 5)))
...
def test_find_matches_maxmatch():
    ...
    D = rng.RNG.random(64)
    for excl_zone in range(3):
        max_matches = rng.RNG.integers(0, 100)
...
def test_gpu_searchsorted():
    ...
    for k in range(1, 32):
        ...
        A = np.sort(rng.RNG.random(size=(n, k)), axis=1)
...
def test_update_incremental_PI_egressFalse():
    ...
    T = rng.RNG.random(64)
    t = rng.RNG.random()  # new datapoint
    T_new = np.append(T, t)
